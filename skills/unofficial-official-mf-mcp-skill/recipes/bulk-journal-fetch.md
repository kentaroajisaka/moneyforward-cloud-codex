...
- `per_page` の上限は **10,000**（2026-04-10 実測確認済み）
...
- **1 会計年度内であれば**一括取得可能（月ごとに分ける必要はない）
- 各ページの結果ファイルを `data/{会社名}/tmp/page_01.txt`, `page_02.txt` に保存

### ⚠️ 会計年度跨ぎの制約（2026-04-19 実測）

`start_date` / `end_date` は **1 つの会計年度内**に収める必要がある。跨いだ指定は 400 エラーになる。

**NG 例**（3 月決算法人で 2 年度分を 1 回で取ろうとした場合）:
```
getJournals(start_date="2024-04-01", end_date="2026-03-31", per_page=10000)
→ {"code":"invalid_query_parameter_value","message":"Given date is not matching any accounting periods."}
```

**OK（会計年度ごとに分割）**:
```
getJournals(start_date="2024-04-01", end_date="2025-03-31", per_page=10000)  # 2024 年度
getJournals(start_date="2025-04-01", end_date="2026-03-31", per_page=10000)  # 2025 年度
```

決算月は会社ごとに異なる（`currentOffice` の `accounting_periods` を先に取得して年度境界を確認すること）。
